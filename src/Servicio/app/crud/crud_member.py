...
class CRUDMember(CRUDBase[Member, MemberCreate, MemberUpdate]):
     ...
     def get_multi_members_from_hive_id(   self, db: Session, *, hive_id:int, limit: int = 100, ) -> List[Member]:
          try:
              return db.query(Member).join(CampaignRole).filter(and_(CampaignRole.hive_id== hive_id,CampaignRole.member_id==Member.id)).limit(limit).all()
     ...
     def get_static_user(   self, db: Session, *, hive_id:int ) -> Member:
          try:
              return db.query(Member).join(CampaignRole).filter(and_(CampaignRole.hive_id== hive_id,CampaignRole.member_id==Member.id,Member.real_user==False)).first()
          except Exception as e:
                        raise HTTPException(status_code=500, detail=f"Error with mysql {e}" )
     ...
     def get_multi_worker_members_from_hive_id( self, db: Session, *, hive_id:int, limit: int = 100) -> List[Member]:
          try:
              return db.query(Member).join(CampaignRole).filter(and_(CampaignRole.hive_id== hive_id, (CampaignRole.role=="WorkerBee" or CampaignRole.role=="QueenBee"),CampaignRole.member_id==Member.id,Member.real_user==True)).limit(limit).all()
          except Exception as e:
                        raise HTTPException(status_code=500, detail=f"Error with mysql {e}" )
