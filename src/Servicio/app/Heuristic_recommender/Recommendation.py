            ...
            if campaign.start_datetime.replace(tzinfo=pytz.timezone('Europe/Madrid')) <= time.replace(tzinfo=pytz.timezone('Europe/Madrid'))  and time.replace(tzinfo=pytz.timezone('Europe/Madrid')) < campaign.end_datetime.replace(tzinfo=pytz.timezone('Europe/Madrid')):
                ...
                if len(list_cells) != 0:
                    for cell in list_cells:
                        point=recipe_in.member_current_location
                        centre= cell.centre
                        distancia = vincenty_inverse((centre['Latitude'], centre['Longitude']), (point['Latitude'], (point['Longitude'])))
...
def create_recomendation_system_per_campaign(
    *,
    ...
    if len(cells_and_priority)==0:
        ...
        return {"detail": "no_measurements_needed"}
    cells_and_priority.sort(
        key=lambda order_features: (order_features[1], -order_features[2]), reverse=True)
    a=[]
    for i in cells_and_priority:
        if i[6].id==campaign_id:
            a.append(i)
...
def nerby_recomendation(
    *,
    ...
    if len(cells_and_priority)==0:
        ...
        return {"detail": "no_measurements_needed"}
    cells_and_priority.sort(
        key=lambda order_features: (-order_features[2], order_features[1]), reverse=True)
    a=[]
    for i in cells_and_priority:
        if i[6].id==campaign_id:
            a.append(i)
...
def nerby_recomendation_with_limit(
    *,
    member_id: int,
    campaign_id:int,
    recipe_in: RecommendationCreate,
