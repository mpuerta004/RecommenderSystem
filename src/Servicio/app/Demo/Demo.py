...
from Mix_recommender import mix_recommender
...
@api_router_demo.post("/campaign_mix", status_code=201, response_model=None)
def asignacion_recursos_hive_mix(
    hive_id:int,
    campaign_id:int,
            ...
            if list_users_available != []:
                for user_class in list_users_available:
                    ...
                    if lat is not None and lon is not None:
                        ...
                        recomendaciones= mix_recommender.create_recomendation(member_id=user_class.member.id,recipe_in=a,db=db,time=time,campaign_id=campaign_id)
