...
def recommendation(message):
    ...
    if member != None and member_2 != None:
        ...
        location_btn = types.KeyboardButton("Share location", request_location=True, one_time_keyboard=True)
        # location_button = types.KeyboardButton(text="Enviar Ubicación en Tiempo Real", request_location=True)
        # markup = types.ReplyKeyboardMarkup([[location_button]], resize_keyboard=True, one_time_keyboard=True)
        
        # update.message.reply_text("Por favor, comparte tu ubicación en tiempo real haciendo clic en el botón de abajo.", reply_markup=reply_markup)
        ...
        bot.send_message(message.chat.id, "In case you can't send the correct location, you can send it with the location option that allows you the clip button.")
...
# # Función para generar la URL de la imagen estática del mapa con marcadores
# def generate_map_image_url(locations):
#     # Construir la URL de la API de Google Maps Static
...
#         "maptype": "roadmap",
#         "format": "png",
#         "markers": '|'.join(f"{loc['latitude']},{loc['longitude']}" for loc in locations),
#         "key": GOOGLE_MAPS_API_KEY
#     }
#     # Construir la URL completa
#     map_url = f"{base_url}?{'&'.join(f'{key}={value}' for key, value in params.items())}"
#     return map_url
    ...
    if user != None:
        ...
        if rec != None:
            ...
            for i in list_notified_recommendation:
                crud.recommendation.remove(db=db, recommendation=i)
            bot.reply_to(message, "It's time for you to send the photo! Please send the photo to me to integrate it into the collage.")
                    ...
                    if data['recommendation_id'] == None:
                        ...
                        if lat != None and long != None:
                            ...
                            if recomendation_aceptada_2 == None:
                                ...
                                bot.reply_to(
                                        message, "Thanks for sending the photo! We are integration it in the map. This may take a few seconds.")
                                crear_mapa(message)
...
def definir_mensajes():
    "Bucle infinito que comprueba si hay nuevos mensajes en el boit"
...
if __name__ == "__main__":
    bot.set_my_commands([
        ...
        telebot.types.BotCommand("/change_personal_information", "Change personal information")
    ])
