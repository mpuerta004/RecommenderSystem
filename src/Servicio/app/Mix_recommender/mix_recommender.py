        ...
        if definitivos!=[]:
                ...
                if len(definitivos)>3:
                    ...
                    for i in range(len(definitivos)):
                        ...
                        result.append(RecommendationCell(
                                recommendation=recomendation, cell=cell))
                List_thredholes_users=crud.bio_inspired.get_list_threshole(db=db, member_id=member_id)
                # print(List_thredholes_users)
                booleano= any(valor[0] < 3.0 for valor in List_thredholes_users)
                if booleano== False or (slot.end_datetime <= time and slot.end_datetime - time <= timedelta(minutes=30)):
                          result= create_recomendation_per_campaign(member_id=member_id,campaign_id=campaign_id,recipe_in=recipe_in,db=db,time=time)
                          print("********************************************VTAE*******************************************************")
                          return result
